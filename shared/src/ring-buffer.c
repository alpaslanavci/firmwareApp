...
void ring_buffer_setup(ring_buffer_t* rb, uint8_t* buffer, uint32_t size) {
    ...
    rb->read_index = 0;
    rb->write_index = 0;
    rb->mask = size - 1; // With the assumption size being a power of 2
}
...
bool ring_buffer_read(ring_buffer_t* rb, uint8_t* byte) { 
    uint32_t local_read_index = rb->read_index; // This and below ensures rb->write and rb->read does not change 
    uint32_t local_write_index = rb->write_index; // beginning from the entry point of this function

    if(ring_buffer_empty(local_read_index, local_write_index)) {
        return false;
    }
